...
A gradient is a smooth transition between two or more colors, used anywhere a solid color or an
...
`radial-gradient(shape, color-stop, ...)` blends outward from a centre point. Named color stops can
each carry a position, like `#F2A93B 40%`, to control exactly where one color ends and the next
begins.
...
## Your Tasks
### 1. Blend two colors in a straight line
...
### 3. Control where a color stop lands
A percentage after a color pins exactly where it reaches full strength.

```
.progress { background: linear-gradient(90deg, #15803D 60%, #EEF1F4 60%); }
```

### 4. Layer three or more colors
...
### 5. Fade an image into a solid color
...
Give a banner a diagonal `linear-gradient` between two brand colors.

### Exercise 2: Radial badge
Build a circular badge with a `radial-gradient` background.

### Exercise 3: Hard-edged progress bar
Use two color stops at the same percentage in a `linear-gradient` to create a sharp edge instead of
a blend, for a simple progress indicator.

### Exercise 4: Three-color gradient
Build a gradient with three color stops and describe in one sentence where each color is strongest.
...
### Q3. What does a percentage after a color, like `red 40%`, control?
1. The color's opacity
2. Where that color stop reaches full strength
3. How many times the color repeats
4. Nothing — percentages are not valid in gradients

### Q4. How many color stops can one gradient have?
1. Exactly two
2. At most three
3. Two or more — there is no fixed limit
4. Only one

### Q5. How would you fade a background image under a translucent color?
